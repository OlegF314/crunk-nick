# Схема Кранка-Николсона для BVP2 солитон + гауссова экспонента
...
from for_reports import relative_l2_norm, get_lw_errs
...
def main(d_config):
    ...
    for i in range(nx):
        matrix[i][i] = 1j - 10 * a6 * lam + 5j * a5 * h * lam + 3 * a4 * h**2 * lam - 1.5j * a3 * h**3 * lam - a2 * h**4 * lam + 0.5j * a1 * h**5 * lam
        matrix[i][i - 1] = 7.5 * a6 * lam - 5j * a5 * h * lam - 2 * a4 * h**2 * lam + 1.5j * a3 * h**3 * lam + 0.5 * a2 * h**4 * lam - 0.5j * a1 * h**5 * lam
        matrix[i - 1][i] = 7.5 * a6 * lam - 2.5j * a5 * h * lam - 2 * a4 * h**2 * lam + 0.5j * a3 * h**3 * lam + 0.5 * a2 * h**4 * lam
        matrix[i][i - 2] = -3 * a6 * lam + 2.5j * a5 * h * lam + 0.5 * a4 * h**2 * lam - 0.5j * a3 * h**3 * lam
        matrix[i - 2][i] = -3 * a6 * lam + 0.5j * a5 * h * lam + 0.5 * a4 * h**2 * lam
        matrix[i][i - 3] = 0.5 * a6 * lam - 0.5j * a5 * h * lam
        matrix[i - 3][i] = 0.5 * a6 * lam
    matrix = np.array(matrix)
    for i in range(nt - 1):
        qcurr = qs[i]
        right_part = np.array([(- (-10 * a6 * lam + 5j * a5 * h * lam + 3 * a4 * h**2 * lam - 1.5j * a3 * h**3 * lam - a2 * h**4 * lam + 0.5j * a1 * h**5 * lam) * qs[i, j]
                                - (7.5 * a6 * lam - 5j * a5 * h * lam - 2 * a4 * h**2 * lam + 1.5j * a3 * h**3 * lam + 0.5 * a2 * h**4 * lam - 0.5j * a1 * h**5 * lam) * qs[i, j - 1]
                                - (7.5 * a6 * lam - 2.5j * a5 * h * lam - 2 * a4 * h**2 * lam + 0.5j * a3 * h**3 * lam + 0.5 * a2 * h**4 * lam) * qs[i, (j + 1) % nx]
                                - (-3 * a6 * lam + 2.5j * a5 * h * lam + 0.5 * a4 * h**2 * lam - 0.5j * a3 * h**3 * lam) * qs[i, j - 2]
                                - (-3 * a6 * lam + 0.5j * a5 * h * lam + 0.5 * a4 * h**2 * lam) * qs[i, (j + 2) % nx]
                                - (0.5 * a6 * lam - 0.5j * a5 * h * lam) * qs[i, j - 3]
                                - (0.5 * a6 * lam) * qs[i, (j + 3) % nx])
                              + 1j * qs[i, j]
                              - tau / 2 * (qs[i, j] * abs(qs[i, j]) ** 2 * (b1 + b2 * abs(qs[i, j]) ** 2 + b3 * abs(qs[i, j]) ** 4) +
                                           qcurr[j] * abs(qcurr[j]) ** 2 * (b1 + b2 * abs(qcurr[j]) ** 2 + b3 * abs(qs[i, j]) ** 4))
                               for j in range(nx)])
    ...
    d = {'x': xs_1d, 't': ts_1d,
         'pred_u': us_1d, 'pred_v': vs_1d, 'pred_h': absqs_1d, 'ic_u': us_1d, 'ic_v': vs_1d}
    df = pd.DataFrame(d)
...
if __name__ == "__main__":
    ...
    args_parser.add_argument("--Ag", default=0.2, type=float)
    args_parser.add_argument("--Bg", default=0., type=float)
    args_parser.add_argument("--Cg", default=5., type=float)
    ...
    args_parser.add_argument("--a1", default=1., type=float)
    args_parser.add_argument("--a2", default=1., type=float)
    args_parser.add_argument("--a3", default=1., type=float)
    args_parser.add_argument("--a4", default=1., type=float)
    args_parser.add_argument("--a5", default=1., type=float)
    args_parser.add_argument("--a6", default=1., type=float)
    args_parser.add_argument("--b1", default=1., type=float)
    args_parser.add_argument("--b2", default=1., type=float)
    args_parser.add_argument("--b3", default=1., type=float)
    args_parser.add_argument("--delta", default=0.001, type=float)
    args_parser.add_argument("--nslices", default=5, type=int)
    ...
    d_config = dict(alpha=alpha, alpha0=alpha0, k=k, w=w, x0=x0, theta=theta, Ag=Ag, Bg=Bg, Cg=Cg,
